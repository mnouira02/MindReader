...
```bash
git clone [https://github.com/mnouira02/MindReader.git](https://github.com/mnouira02/MindReader.git)
cd MindReader
...
```text data/ raw_data/ # BIDS format EEG things_images/ # Unzipped image folders ```

3. Preprocessing
Convert raw BIDS data to optimized NumPy arrays:

```bash python src/convert_data.py ```

4. Running Inference (Pre-trained)
If you have downloaded the pre-trained weights (link coming soon), run:

```bash python src/generate_thoughts.py --model_path checkpoints/mind_reader_adapter.pth ```
